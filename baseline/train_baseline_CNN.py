#!/usr/bin/env python3
"""
Minimal supervised geo-localization baseline: CNN -> (lat, lon) regression.
Uses a pretrained ResNet from timm; same data and training setup as train.py (ViT).
...
class CNNGeo(nn.Module):
    """Pretrained CNN backbone (e.g. ResNet50) with linear head for (lat, lon) regression."""
...
def main():
    ...
    parser.add_argument("--save", type=str, default="baseline_checkpoint.pt", help="Path to save model checkpoint (for RL --baseline_checkpoint). Use '' to skip saving.")
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (imgs, targets) in enumerate(train_loader):
            ...
            if (batch_idx + 1) % 10 == 0 or (batch_idx + 1) == n_batches:
                print(f"  Epoch {epoch + 1}/{args.epochs}  batch {batch_idx + 1}/{n_batches}  loss={loss.item():.4f}")
        train_loss_avg = train_loss_sum / n_batches
        print(f"  Epoch {epoch + 1}/{args.epochs}  train loss (avg): {train_loss_avg:.4f}  time: {time.perf_counter() - epoch_start:.1f}s")
